...
public class Sorts : MonoBehaviour
{
    ...
    IEnumerator BubbleSortAlgorithm(GameObject[] unsortedList)
    {
        ...
        while (!done)
        {
            ...
            for (k = 0; k < unsortedList.Length - sorted - 1; k++)
            {
        ...
        {
            yield return new WaitForSeconds(speed);
            for (k = 0; k < unsortedList.Length - sorted; k++)
            {
                SetColor(unsortedList[k], 1);
                unsortedList[k].GetComponentInChildren<ParticleSystem>().Play();
            }
